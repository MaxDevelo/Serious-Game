...
- Proposition 1
Modifier le visuel pour le rendre plus attrayant et ergonomique tel que la mise en place de jauges au lieu de simples pourcentages, l'année constament affiché plutôt que de laisser la possibilité de la dérouler, ou encore un train de marchandise passant de temps en temps sur les rails pour ajouter de la vie.

- Proposition 2
Ajouter la possibilité d'influencer les probabilités de propagation des differentes activitées, soit par un batiment dédié soit via des synergies entre certains batiments. De plus, cette solution ajoute de la rejouabilité au jeu puisqu'il faudra alors découvrir les differentes synergies.

-Proposition 3
Ajuster certains petits éléments pour que la compréhension soit plus simple pour le joueur tel qu'ajouter des "+" dans le recapitulatif des tours ou mettre un petit symbole clignotant indiquant la touche pour passer le dialogue dans le tutoriel.
...
Justifications :
J'ai verifié les differents droits et et l'historiques des commits dans leur dépot git. Pour ce qu'il est des commits la grande majorité sont pertinents mais les derniers on l'air d'être fait d'avantage à la va vite (sans description précise et plus fréquent). Quant au README il est très bien constitué et complet.
...
Justifications :
L'installation du projet sur ma machine s'est effectué sans encombre (j'ai suivi les instructions écrites dans le README). Après quelques test le jeu ne s'est jamais fermé inopinément cependant les dimensions de la fenêtre de jeux à eu un peu de mal à s'adapter à la taille de mon écran. Le jeu contient un bref tutoriel intégré à la partie qui permet de comprendre les interactions possibles. Cependant le prototype ne permet pas de saisir complètement les conceptes qui ont voulu etre transmis, mais l'utilisateur assimile tout de même l'idée générale de la reterritorialisation.
...
Justifications : 
Après comparaison avec le modèle du documents, j'ai pu observer que le CdC ne respecte pas complètement le modèle du document. Par exemple dans la partie "Description des objectifs pédagogiques" la partie concernant les "Membres d’un projet tuteuré" n'apparait pas. Malgré tout le CdC est très détaillé voir même plus détaillé que dans le jeu puisque toutes les données chiffrés sont donnés. Les informations concernant les fonctionnalitées et le gameplay y sont également exposés. Le modèle conceptuel est bien fournit et bien organisé.
...
Justifications : 
Les fonctionnalitées sont peu nombreuses puisque l'essentiel du jeu consiste en une même fonctionnalité cependant cette dernière est très bien introduite aussi bien au travers des actions du joueur que de la description des informations. Le fonctionnement d'un tour est très détaillé et agrémenté d'un exemple dans le README. Le jeu met bien en avant la fonctionnalité principale cependant la plupart des fonctionnalités qui pourrait être présente n'ont pas pu voir le jour (faute de temps) mais sont écrite dans leur TODO. Etant donné l'absence d'autre mode de jeu ou de selection de difficulté les fonctionnalités possible ne sont pas annotée en difficulté.
...
Justifications :
Le scénario principal est exposé dans le tutoriel et amène, après quelques tours, soit à la défaite, soit à la victoire du joueur. Pour arriver jusqu'à une des fins il est nécessaire de se confronter à toutes les fonctionnalités (pas forcément tout les éléments mais bien toutes les fonctionnalités). Aucun autre scénario n'est disponible.
